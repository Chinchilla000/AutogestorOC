...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sentencia = $conexion->prepare("SELECT ro.*, e.nombre_empresa FROM residentes_obra ro
                                     JOIN empresas e ON ro.id_empresa = e.id
                                     WHERE ro.correo = :correo");
    $sentencia->bindParam(':correo', $correo);
    $sentencia->execute();
    $residente = $sentencia->fetch(PDO::FETCH_ASSOC);

    if ($residente && password_verify($contrasena, $residente['contrasena'])) {
        // Inicio de sesión exitoso, iniciar la sesión y redirigir al área de residentes de obra
        session_start();
        ...
        header("Location: iniciar_sesion_residente_obra.php?error=1");
        exit();
    }
}
?>
...
<head>
    ...
    <title>Iniciar Sesión Residente de Obra</title>
...
<body class="bg-light d-flex flex-column min-vh-100">
    ...
    <main class="form-signin w-100 m-auto container flex-grow-1">
        <form action="iniciar_sesion_residente_obra.php" method="post">
            <img class="mb-4" src="../../../img/logo.png" alt="Logo" width="72" height="57">
            <h1 class="h3 mb-3 fw-normal">Iniciar Sesión: Residente de Obra</h1>
